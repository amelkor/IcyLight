...
internal static class Program
    {
        private static readonly ILogger _log = new LoggerConfiguration().WriteTo.Console().MinimumLevel.Verbose().CreateLogger();
        ...
        private static void RunKeysCreation(KeyOptions options)
        {
            try
            {
                ...
                File.WriteAllText(privPath, JsonSerializer.Serialize(privateKey));
                File.WriteAllText(pubPath, JsonSerializer.Serialize(publicKey));
        ...
        private static void RunPackaging(PackOptions options)
        {
            ...
            if (Directory.Exists(options.OutPath) && Directory.GetFiles(options.OutPath).Any())
            {
            ...
            _log.Information("Package creation started");
            _log.Information($"Source path: {options.SourcePath}");
            _log.Information($"Output path: {options.OutPath}");
